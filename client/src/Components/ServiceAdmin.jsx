...
export default function ServiceAdmin({ thumbnail, name, _id, url, handleEdit, handleDelete, uploading }) {
  ...
  const [service, setservice] = useState({ name: "", url: "", thumbnail: "" })
  ...
  const handleEditService = (id, data) => {
    handleEdit(id, data);
    if (!uploading)
      setShow(false)
  }
            ...
            <div className="relative p-4 w-full max-w-md max-h-full">
              ...
              <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div className="p-3 space-y-4">
                  ...
                  <div >
                    <div className="block mb-2 text-sm font-medium text-gray-900 dark:text-white" htmlFor="file_input">Thumbnail</div>
                    <div className="preview mx-auto my-4">
                      <img src={service.thumbnail} className={`w-20 h-20 ${!thumbnail ? "invisible" : ""}`} alt="" />
                    </div>
                  </div>

                  <button onClick={() => handleEditService(_id, service)} disabled={uploading} className=" text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-32 h-10 flex justify-center items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 disabled:bg-gray-400" type="button" >
                    {uploading ? <>
